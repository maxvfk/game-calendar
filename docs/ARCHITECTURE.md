...
## Current deployment (Account Sync S5)
...
The S3 PostgreSQL tables/RLS/RPC accept the explicit first-login import and
S5's conditional sync mutations. Account edits first enter the scoped local
cache and durable outbox; startup, foreground, online, periodic and debounced
cycles pull, merge, push and confirm the cloud winner. Signed-out use stays on
a separate local guest profile. See the approved account-sync design and
`supabase/README.md`.

The Bun/SQLite architecture below is older reference design, not the current
GitHub Pages deployment or the S5 account implementation.
...
src/
  ...
  ingest/
    ...
    parsers/
      ...
      bawiki.ts         bluearchive.wiki schedule (#eventtable & tabber) ✓ built
